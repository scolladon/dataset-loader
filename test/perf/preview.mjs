...
const throughputRuntime = runtime.filter(b => b.name.startsWith('throughput-'))
const phaseRuntime = runtime.filter(b => !b.name.startsWith('throughput-'))
const throughputMemory = memory.filter(b => b.name.startsWith('throughput-'))
const phaseMemory = memory.filter(b => !b.name.startsWith('throughput-'))
...
<head>
  <meta charset="UTF-8">
  <title>dataset-loader — Performance Benchmarks</title>
...
<body>
  <h1>dataset-loader — Performance Benchmarks</h1>
  ...
  <div class="grid">
    <div class="chart-box">
      <h2>Throughput — Runtime (ops/sec, higher is better)</h2>
      <canvas id="throughputRuntime"></canvas>
    </div>
    <div class="chart-box">
      <h2>Throughput — Mean Time (ms, lower is better)</h2>
      <canvas id="throughputMemory"></canvas>
    </div>
  </div>
  ...
  <div class="grid">
    <div class="chart-box">
      ...
      <table>
        ...
        ${throughputRuntime.map((r, i) => `<tr><td>${r.name}</td><td class="val">${r.value.toLocaleString()}</td><td class="val">${throughputMemory[i].value}</td><td class="val">${r.range}</td></tr>`).join('\n        ')}
      </table>
    </div>
    <div class="chart-box">
  ...
  <script>
    ...
    bar('throughputRuntime', ${JSON.stringify(throughputRuntime.map(b => b.name))}, ${JSON.stringify(throughputRuntime.map(b => b.value))}, 'log');
    bar('throughputMemory',  ${JSON.stringify(throughputMemory.map(b => b.name))},  ${JSON.stringify(throughputMemory.map(b => b.value))}, 'log');
