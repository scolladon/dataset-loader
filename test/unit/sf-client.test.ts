...
import { SfClient, type QueryResult } from '../../src/core/sf-client.js'
...
describe('SfClient', () => {
  ...
  beforeEach(() => {
    requestSpy = vi.fn()
    sut = new SfClient(mockConnection(requestSpy), { retryBaseDelayMs: 0 })
  })

  describe('query', () => {
    it('given a SOQL string, when querying, then sends GET with encoded SOQL and gzip header', async () => {
      // Arrange
      const expected: QueryResult<{ Id: string }> = { totalSize: 1, done: true, records: [{ Id: '001' }] }
  ...
  describe('queryMore', () => {
    it('given a nextRecordsUrl, when querying more, then sends GET to that URL', async () => {
      // Arrange
      const expected: QueryResult<{ Id: string }> = { totalSize: 0, done: true, records: [] }
      requestSpy.mockResolvedValue(expected)
  ...
  describe('getBlob', () => {
    it('given a blob path, when fetching, then sends GET with gzip header', async () => {
      ...
      // Act
      const result = await sut.getBlob('/services/data/v62.0/sobjects/EventLogFile/0AT/LogFile')
  ...
  describe('post', () => {
    it('given a path and body, when posting, then sends POST with JSON body and gzip headers', async () => {
      ...
      // Act
      const result = await sut.post('/services/data/v62.0/sobjects/InsightsExternalData', body)
  ...
  describe('patch', () => {
    it('given a path and body, when patching, then sends PATCH with JSON body', async () => {
      ...
      // Act
      await sut.patch('/services/data/v62.0/sobjects/InsightsExternalData/06V', body)
  ...
  describe('retry', () => {
    ...
    it('given HTTP 429 on all 3 attempts, when querying, then throws after exhausting retries', async () => {
      ...
      // Act & Assert
      await expect(sut.query('SELECT Id FROM Account')).rejects.toThrow('Too Many Requests')
      expect(requestSpy).toHaveBeenCalledTimes(3)
    })
    ...
    it('given non-429 error, when querying, then throws immediately without retry', async () => {
      ...
      // Act & Assert
      await expect(sut.query('SELECT Id FROM Account')).rejects.toThrow('Server Error')
