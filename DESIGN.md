...
- **DateBounds** — Run-scoped CLI `--start-date` / `--end-date` window. Two-layer validation (ISO regex via `Watermark.fromString` + round-trip UTC-components calendar check). Exposes `lowerConditionFor(dateField, watermark)` / `upperConditionFor(dateField)` for reader SOQL assembly and `rewindsBelow(watermark)` / `leavesHoleAbove(watermark)` / `matchesWatermark(watermark)` / `endsBeforeWatermark(watermark)` predicates used by the command layer to emit REWIND / HOLE / BOUNDARY / EMPTY warnings. Rule: **`--start-date` always wins when set** (CLI overrides watermark on the lower bound); the watermark fills in only when `--start-date` is absent.
...
A dataset whose `EdgemartAlias` has no `Completed` / `CompletedWithWarnings` prior load is **bootstrapped** on first upload: the writer synthesises the required `MetadataJson` from the source schema and CRM Analytics creates the dataset on `Action: Process`. Bootstrap is **always-on** — there is no opt-in flag and the user controls intent through `targetDataset` naming and the dry-run `[BOOTSTRAP]` annotation. Full spec: `docs/design/2026-05-18-bootstrap-new-dataset-upload.md`.
...
`SourceSchema` is reader-agnostic. The synthesiser is the **single** place that knows the CRMA wire format; providers know their source quirks but never touch the wire. Schema mapping per reader kind:

- **SObject** — `SObjectBootstrapProvider` walks SF Describe per dotted field path (`Owner.Profile.Name` → `Owner_Name → Profile_Name`), maps SF types to CRMA types (`int/double/currency/percent → Numeric`, `date/datetime → Date`, everything else → `Text`, including `boolean` per CRMA convention). Describe responses are cached per-instance + per-sObject.
- **ELF** — `ElfBootstrapProvider` queries the latest `EventLogFile.LogFileFieldNames`, emits all-`Text` (ELF wire format is textual). Fails fast if no prior log row exists.
- **CSV** — `CsvBootstrapProvider` reads the file's first line, emits all-`Text`. Type inference is **not** attempted — locale-dependent dates and ambiguous nullability sentinels make it a UX trap.

### Wire-grounded constraints (CRMA v67.0)

Every default in `synthesise()` is anchored to a probe that exercised the CRMA API:

- **Numeric** — `precision` clamped to `[1, 18]` (SF Describe sometimes returns `precision: 0`; CRMA rejects), `scale` clamped to `≤ precision`, mandatory `defaultValue: "0"` (CRMA rejects parent-create otherwise with `FIELD_INTEGRITY_EXCEPTION`).
- **Date** — `defaultValue` is **not** emitted (CRMA accepts empty cells as null); default `format: "yyyy-MM-dd'T'HH:mm:ss.SSSXXX"` matches SF native datetime (`+0000` offset — CRMA's `XXX` token rejects colon-separated `+00:00` and the `Z` literal). Date-only fields use `format: "yyyy-MM-dd"`.

### Override path

The per-entry `overrideMetadata: boolean` flag forces re-synthesis on subsequent runs, replacing the dataset's canonical schema. The config-loader rejects `overrideMetadata: true` paired with `operation: 'Append'` at parse time — CRMA enforces strict schema-match on Append (any type, defaultValue, or field-set deviation surfaces as `FIELD_INTEGRITY_EXCEPTION` at parent-create), so the only safe pairing is `operation: 'Overwrite'`. A second config-loader rule (`validateOverrideMetadataConsistency`) ensures all entries fanning out to the same dataset agree on the flag.

### Writer + audit integration
...
`datasetReady` audit returns `WARN` (not `FAIL`) when no prior completed load exists — the dry-run `[BOOTSTRAP]` annotation is the user's last visible signal against typo'd dataset names. `schemaAlignment` consults the synthesised schema in two cases (metadata absent OR `overrideMetadata: true`), running the same order/set/augment-overlap checks against the synthesised fields. Providers are injected into the domain audit through `AuditContext.bootstrapProviderFor` from the application layer — the domain never imports adapter classes.

For SObject entries, bootstrap produces an **identity** `ProjectionLayout`: synthesised `datasetFields` order equals the reader's order, so `outputIndex[i] = i`. The existing column-alignment projection path handles it transparently — no new code path.

### Dry-run annotations

| Annotation     | Trigger                                                                | Worst-case typo cost |
|----------------|------------------------------------------------------------------------|----------------------|
| `[BOOTSTRAP]`  | `datasetReady` returned WARN (no prior completed load)                 | Ghost dataset        |
| `[OVERRIDE]`   | `overrideMetadata: true` + dataset already exists                      | Data loss            |
...
| `DatasetWriterFactory` | `adapters/writers/dataset-writer.ts`     | Creates `DatasetWriter` instances per CRM Analytics dataset. `DatasetWriter.init()` resolves metadata via a two-branch chain (existing → synthesised) — falls back to the injected `BootstrapMetadataProvider` when no prior completed load exists or `overrideMetadata: true` is set. Normalizes `numberOfLinesToIgnore` to 0, creates the parent record, and returns a `GzipChunkingWritable`. The writable batch-compresses with 512KB flush threshold, splits at 10 MB base64 boundaries, and keeps at most `UPLOAD_HIGH_WATER` (= 25) part uploads in-flight via `Promise.race` backpressure. Emits `info` "Bootstrapping new dataset" + `debug` synthesised MetadataJson on the synthesised path. |
| Bootstrap providers    | `adapters/readers/{sobject,elf,csv}-bootstrap-provider.ts` | Per-reader `BootstrapMetadataProvider` implementations that build a `SourceSchema` from the source. SObject walks SF Describe per dotted field path with per-instance caching; ELF queries `EventLogFile.LogFileFieldNames`; CSV reads the file's first line. All three emit augment columns as `Text` via the shared `appendAugmentFields` helper. |
| `MetadataSynthesizer`  | `domain/metadata-synthesizer.ts`         | Pure `SourceSchema → CRMA MetadataJson` mapping. Anchors every wire-format default (Numeric precision/scale clamps + mandatory `defaultValue: "0"`, Date `format: yyyy-MM-dd'T'HH:mm:ss.SSSXXX` with `XXX = ±hhmm` only) to a specific CRMA v67.0 probe documented inline. Rejects invalid dataset names, duplicate field names (case-insensitive), and untranslated dotted field paths with `SynthesizeError`. |
...
| `Auditor`              | `domain/auditor.ts`                      | Builds and runs pre-flight checks (`authConnectivity`, `elfAccess`, `insightsAccess`, `sobjectReadAccess`, `datasetReady`, `schemaAlignment`). Deduplicates by `(org, key)` across entries. `MetadataJson` is memoised per `(org, dataset)` within a single audit run. `sobjectReadAccess` enforces FLS via `WITH SECURITY_ENFORCED`; on entities Salesforce blocks (e.g. `User`, `UserLogin`) it re-probes without the clause and reports `[WARN]` so the audit doesn't fail on entities where the platform won't let us assert FLS. `datasetReady` emits `WARN` (not `FAIL`) on missing datasets — bootstrap is always-on, the dry-run `[BOOTSTRAP]` annotation is the user-visible signal. `schemaAlignment` consults the synthesised schema when metadata is absent OR `overrideMetadata: true`, with the bootstrap provider injected via `AuditContext.bootstrapProviderFor` from the application layer (domain never imports adapters). `runAudit` executes all checks via `Promise.allSettled`, logs `[PASS]`/`[WARN]`/`[FAIL]` per check, and returns `passed: boolean` (WARN is non-blocking). |
